...
export default function AdminIncomePage() {
  ...
  return <div className="space-y-6"><div className="rounded-3xl bg-[var(--brand-ink)] px-6 py-7 text-white shadow-xl shadow-slate-900/10 lg:px-8"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--brand-gold)]">Money overview</p><h1 className="mt-2 text-3xl font-black tracking-tight">Income</h1><p className="mt-1 max-w-2xl text-sm text-slate-300">Showing all users. All approved deposit history, accepted withdrawals, and paid referral rewards are included. No records are deleted.</p></div>{error && <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{cards.map((card) => { const Icon = card.icon; return <Card key={card.label} className="!p-5"><div className={`mb-5 flex h-11 w-11 items-center justify-center rounded-2xl ${card.tone}`}><Icon className="h-5 w-5" /></div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">{card.label}</p><p className={`mt-2 text-2xl font-black ${card.value < 0 ? "text-red-600" : "text-slate-900"}`}>{formatCurrency(card.value)}</p><p className="mt-1 text-xs text-slate-500">{card.count}</p></Card>; })}</div><Card title="Per-user calculation" subtitle="Every user with approved financial activity is included."><div className="space-y-3">{income.users.map((user) => <div key={user.username} className="rounded-2xl border border-slate-100 bg-slate-50 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-black text-slate-900">{user.username}</p><p className={`font-black ${user.remainingAmount < 0 ? "text-red-600" : "text-[var(--brand-teal)]"}`}>{formatCurrency(user.remainingAmount)} remaining</p></div><div className="mt-3 grid gap-2 text-sm sm:grid-cols-3"><span className="text-emerald-700">Received: {formatCurrency(user.totalInvestments)}</span><span className="text-orange-700">Withdrawn: {formatCurrency(user.totalWithdrawals)}</span><span className="text-fuchsia-700">Referral: {formatCurrency(user.totalReferralRewards)}</span></div></div>)}</div></Card><Card title="Account calculation" subtitle="The remaining figure is calculated from all approved records."><div className="flex flex-wrap items-center gap-3 text-sm font-bold"><span className="rounded-xl bg-emerald-50 px-3 py-2 text-emerald-700">{formatCurrency(income.totalInvestments)} received</span><span className="text-slate-400">−</span><span className="rounded-xl bg-orange-50 px-3 py-2 text-orange-700">{formatCurrency(income.totalWithdrawals)} withdrawals</span><span className="text-slate-400">−</span><span className="rounded-xl bg-fuchsia-50 px-3 py-2 text-fuchsia-700">{formatCurrency(income.totalReferralRewards)} rewards</span><span className="text-slate-400">=</span><span className="rounded-xl bg-blue-600 px-3 py-2 text-white">{formatCurrency(income.remainingAmount)} remaining</span></div></Card></div>;
}