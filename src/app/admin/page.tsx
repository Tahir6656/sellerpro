    ...
    <div className="space-y-6">
      ...
      {stats ? <div className="grid gap-5 lg:grid-cols-2">
        <Card title="Transaction flow" subtitle="Real activity across the last 14 days">
          <div className="flex h-48 items-end gap-1.5 border-b border-l border-slate-100 px-2 pb-1 pt-4">{stats.transactionChart.map((day, index) => { const max = Math.max(1, ...stats.transactionChart.flatMap((item) => [item.deposits, item.withdrawals])); return <div key={day.date} className="group flex h-full flex-1 flex-col items-center justify-end gap-1" title={`${day.date}: ${formatCurrency(day.deposits)} in, ${formatCurrency(day.withdrawals)} out`}><div className="flex h-full items-end gap-0.5"><div className="w-2 rounded-t bg-[var(--brand-teal)] transition-all group-hover:opacity-75" style={{ height: `${Math.max(3, (day.deposits / max) * 100)}%` }} /><div className="w-2 rounded-t bg-[var(--brand-coral)] transition-all group-hover:opacity-75" style={{ height: `${Math.max(3, (day.withdrawals / max) * 100)}%` }} /></div><span className="text-[9px] text-slate-400">{index % 2 === 0 ? day.date.slice(5) : ""}</span></div>; })}</div>
